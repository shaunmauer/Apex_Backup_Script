...
BEGIN
  ...
  loop
    ...
    "&apex_schema".wwv_flow_gen_api2.export(
      p_flow_id=> &APP_ID,
...
Prompt --Exporting application &APP_ID from workspace &WORKSPACENAME
...
Prompt --Exporting themes for application &APP_ID from workspace &WORKSPACENAME
...
declare
  ...
  L_COUNT NUMBER := 0;
begin
  for r1 in (select theme_id from "&apex_schema".wwv_flow_themes where flow_id = &APP_ID order by theme_id)
  loop
    L_NAME_LIST (1) := 'REQUEST_CHARSET';
    L_VALUE_LIST(1) := 'AL32UTF8';
    L_NAME_LIST (2) := 'REQUEST_IANA_CHARSET';
    ...
    "&apex_schema".wwv_flow_gen_api2.export_theme(
      p_flow_id=>&APP_ID,
      p_commit=>'YES',
      p_format => 'UNIX',
...
Prompt --Exporting application processes for application &APP_ID from workspace &WORKSPACENAME
...
begin
  ...
  for r1 in (select APPLICATION_PROCESS_ID, PROCESS_NAME name from APEX_APPLICATION_PROCESSES where application_id = &APP_ID)
  loop
    "&apex_schema".wwv_flow_gen_api2.export(
      p_flow_id=> &APP_ID,
      p_page_id=>null,
      ...
      p_component_id => r1.APPLICATION_PROCESS_ID
    );
    L_COUNT := L_COUNT + 1;
  end loop;
  ...
  "&apex_schema".wwv_flow_gen_api2.file_close(
            p_commit              => true,
